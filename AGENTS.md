...
ユーザー承認により、実績収集のみGoogle Apps Scriptを利用する。教材本体は静的・依存ゼロを維持する。`js/reporting.js` はstateをimportしない独立した送信待ち管理、`js/views/reporting.js` は結果・設定画面との接続、`integrations/google-apps-script/Code.gs` はGoogle側の受信処理。送信先URLは明示設定された外部接続先なので、静的アセットの相対パス規約の対象外。保存成功と実績ID一致の応答を受け取るまで送信待ちを消さない。Google側は実績IDによる重複排除を必須とする。受信スクリプトのローカル検証と、公開済みGoogle環境での実受信確認を区別して報告する。
