...
test("save→load 往復で totalScore/badges(Set)/cards(Set)/settings が復元される", () => {
  ...
  state.settings = { ...DEFAULT_SETTINGS, sound: false };
  save();
...
test("旧データ互換: settings/bossCleared が欠落したJSONでも DEFAULT_SETTINGS がマージされ bossCleared={} になる", () => {
  localStorage.clear();
  localStorage.setItem(
    ...
    JSON.stringify({
      ...
      cards: [],
      title: "",
      // settings, bossCleared は意図的に欠落（旧バージョンのセーブデータを模擬）
    })
  );
  load();
