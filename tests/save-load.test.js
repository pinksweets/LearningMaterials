...
test("LEAP Basicのグループは初回のみ折りたたみ、開いた状態も保存できる", () => {
  const subject = "📘 LEAP Basic 必携英単語";
  const group = "Part 1｜コース1 見出語・意味・ニュアンス";
  localStorage.clear();
  state.subjectGroupCollapsed = {};

  assert.equal(isSubjectGroupCollapsed(subject, group), true);
  setSubjectGroupCollapsed(subject, group, false);
  assert.equal(isSubjectGroupCollapsed(subject, group), false);
