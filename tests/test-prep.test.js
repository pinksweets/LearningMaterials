...
test('テスト対策は追加順の降順、当日は過去扱いにしない',()=>{
  ...
  assert.deepEqual(testPrepCategories('2026-09-12').map(c=>c.past),[false,false,true]);
  assert.deepEqual(testPrepCategories('2026-09-16').map(c=>c.past),[false,true,true]);
  assert.equal(testPrepCategories('2026-09-16')[0].status,'今日のテスト');
  assert.ok(testPrepCategories('2026-09-17').every(c=>c.past));
