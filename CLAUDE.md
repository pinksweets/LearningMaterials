...
機能テストは node で registry.js → data/*.js（index.html の記載順）→ app.js を連結評価し、`document`/`localStorage` のスタブを与えて採点関数・SRS・保存復元を直接呼ぶ方式が実績あり。DOM 込みの検証は jsdom か Claude Preview（http 配信でも壊れない設計）を使う。
